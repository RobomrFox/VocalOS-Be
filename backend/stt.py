...
class VoiceSignature:
    ...
    def verify(self, enrolled_embedding, audio_np_to_check, threshold=0.65):
            """
            Verifies a speaker by comparing their enrolled_embedding
            against a newly provided audio numpy array (audio_np_to_check).
            """
            print("Verifying speaker...")

            try:
                # 1. Get embedding from the audio that was passed in
                test_embedding = self.get_embedding(audio_np_to_check)
                
                if np.all(test_embedding == 0) or test_embedding is None:
                    print("Failed to create a valid embedding for the test audio.")
                    return False

                # 2. Calculate cosine similarity
                similarity = np.dot(enrolled_embedding, test_embedding) / (np.linalg.norm(enrolled_embedding) * np.linalg.norm(test_embedding))
                
                print(f"Speaker similarity: {similarity:.3f} (Threshold: {threshold})")
                
                # 3. Return True/False based on threshold
